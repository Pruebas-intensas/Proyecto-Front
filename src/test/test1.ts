...
async function crearProductoTest(driver: WebDriver) {
    try{
        ...
        if (currentUrl === environment.url_front+'/home'){
            ...
            await driver.wait(until.elementLocated(By.id('modal-agregar-producto')), 5000);
            ...
            await driver.findElement(By.id('fechaTermino')).sendKeys('31122023');
...
async function editarProductoTest(driver: WebDriver) {}
...
async function executeTests() {
    ...
    let testCounter = 0;
    let testFunctions = [loginTest, registerTest, crearProductoTest];
